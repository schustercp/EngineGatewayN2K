...
void RTC_clock::set_time (const char* time)
{
  _hour   = conv2d(time);
...
unsigned long RTC_clock::get_clock ()
{
  ...
  RTC_GetDate (RTC, &year, &month, &day, NULL);

  days += day;

  do
  {
    month--; //offeset month to be zero based.
    if (month == 1) // february
      days += daysInMonth[month] + switch_years(year);
    else
      days += daysInMonth[month];
  }
  while(month > 0);

  year--; //Remove the current year.

  while(year > 1970)
  {
    days += (365 + switch_years(year));
    year--;
  }
...
// Based on https://github.com/adafruit/RTClib/blob/master/RTClib.cpp
void RTC_clock::set_date (const char* date)
{
  _day = conv2d(date + 4);
  ...
  switch (date[0]) {
    ...
    case 'F': this->_month =  2; break;
    case 'A': this->_month = date[2] == 'r' ? 4 : 8; break;
    case 'M': this->_month = date[2] == 'r' ? 3 : 5; break;
    case 'S': this->_month =  9; break;
    case 'O': this->_month = 10; break;
    case 'N': this->_month = 11; break;
    case 'D': this->_month = 12; break;
  }
