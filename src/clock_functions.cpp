...
uint32_t CLOCK_GetPllFreq(clock_pll_t pll)
{
    ...
    switch (pll)
    {
        ...
        case kCLOCK_PllAudio:
            ...
            {
                case CCM_ANALOG_MISC2_AUDIO_DIV_MSB | CCM_ANALOG_MISC2_AUDIO_DIV_LSB:  //not there
                    freq >>= 2U;
                    break;

                case CCM_ANALOG_MISC2_AUDIO_DIV_LSB:  //not there
                    freq >>= 1U;
                    break;
...
void CLOCK_SetAHBDiv(uint32_t value)
{
    uint32_t busyShift, divider;
    
    divider = CCM_TUPLE(
        CBCDR, CCM_CBCDR_AHB_PODF_SHIFT, 0x1C00U, 1U);
  
    busyShift = CCM_TUPLE_BUSY_SHIFT(divider);
    CCM_TUPLE_REG(CCM, divider) = (CCM_TUPLE_REG(CCM, divider) & (~CCM_TUPLE_MASK(divider))) |
                                  (((uint32_t)((value) << CCM_TUPLE_SHIFT(divider))) & CCM_TUPLE_MASK(divider));
    Serial.printf("%d   /  %d\r\n", busyShift, CCM_NO_BUSY_WAIT);
    //assert_(busyShift <= CCM_NO_BUSY_WAIT);
